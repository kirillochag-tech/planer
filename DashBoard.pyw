...
from PySide6.QtWidgets import (QMainWindow, QMenu, QApplication, QWidget,
                               QHBoxLayout, QVBoxLayout, QPushButton, QDateEdit, QLabel,
                               QMessageBox)
...
class MyApp(QMainWindow):
    ...
    def __init__(self):
        ...
        self.target_percent = 0  # Требуемый процент выполнения
        self.saved_window_geometry = None  # Для сохранения геометрии окна
        self.set_geometry()
    ...
    def setup_history_controls(self):
        """Добавляет элементы управления историей в интерфейс."""
        # На этом этапе self.tabBook уже создан и является центральным виджетом
        existing_central_widget = self.centralWidget()
        assert existing_central_widget is not None, "Центральный виджет должен быть установлен в set_geometry"
        ...
        # Создаем новый центральный виджет, который будет содержать панель управления и вкладки
        new_central_widget = QWidget()
        new_main_layout = QVBoxLayout()
        new_main_layout.addLayout(self.history_control_layout)
        new_main_layout.addWidget(existing_central_widget)  # Это self.tabBook

        new_central_widget.setLayout(new_main_layout)
    ...
    def load_historical_data(self):
        ...
        # === FINAL DEBUG: Проверка структуры сетевой БД ===
        from bin.database_manager import DatabaseManager
        db_man = DatabaseManager()
        print(f"[FINAL DEBUG] Путь к БД: {db_man.db_path}")
        try:
            with db_man._get_connection() as conn:
                cursor = conn.cursor()
                cursor.execute("PRAGMA table_info(sales_data);")
                cols = [row[1] for row in cursor.fetchall()]
                print(f"[FINAL DEBUG] Колонки в sales_data: {cols}")
                
                cursor.execute("SELECT COUNT(*) FROM sales_data;")
                total_count = cursor.fetchone()[0]
                print(f"[FINAL DEBUG] Всего записей в sales_data: {total_count}")
                
                cursor.execute("SELECT manager, cut_manager FROM sales_data WHERE tab_type='managers_26bk' LIMIT 3;")
                samples = cursor.fetchall()
                print(f"[FINAL DEBUG] Примеры данных (manager, cut_manager): {samples}")
        except Exception as e:
            print(f"[FINAL DEBUG] Ошибка при проверке БД: {e}")
        # === END DEBUG ===
        ...
        self.saved_window_geometry = self.saveGeometry()
        ...
        # Получаем target_percent из первой записи (он должен быть одинаковым для всех)
        if historical_data:
            raw_percent = historical_data[0].get('target_percent')
            # Обрабатываем None и пустые значения
            if raw_percent is not None and raw_percent != '':
                try:
                    self.target_percent = float(raw_percent)
                except (ValueError, TypeError):
                    self.target_percent = 0
            else:
                self.target_percent = 0
            print(f"[DEBUG TITLE] Получен target_percent из БД: {self.target_percent}")
        else:
            self.target_percent = 0
            print(f"[DEBUG TITLE] Нет исторических данных, target_percent = 0")
        ...
        if self.saved_window_geometry:
            self.restoreGeometry(self.saved_window_geometry)
    ...
    def load_current_data(self):
        ...
        # Восстанавливаем геометрию, если она была сохранена
        if self.saved_window_geometry:
            self.restoreGeometry(self.saved_window_geometry)
    ...
    def update_title_bar(self):
        ...
        if self.historical_date:
            # Используем сохраненный target_percent для исторических данных
            print(f"[DEBUG TITLE] Формирование заголовка. historical_date={self.historical_date}, target_percent={getattr(self, 'target_percent', 'NOT_SET')}")
            if hasattr(self, 'target_percent') and self.target_percent:
                title = f"Планерка - Исторические данные за {self.historical_date.strftime('%d.%m.%Y')} [Требуемый % выполнения {self.target_percent}]"
            else:
                title = f"Планерка - Исторические данные за {self.historical_date.strftime('%d.%m.%Y')}"
            self.setWindowTitle(title)
            print(f"[DEBUG TITLE] Установлен заголовок: {title}")
        else:
            # Получаем требуемый процент из файла для текущих данных
            try:
                with open('files/total_plan.txt', 'r') as f:
                    percent = f.read().strip()
                    if percent:
                        title = f"Планерка - Текущие данные [Требуемый % выполнения {percent}]"
                    else:
                        title = "Планерка - Текущие данные"
            except FileNotFoundError:
                title = "Планерка - Текущие данные"
            self.setWindowTitle(title)
            print(f"[DEBUG TITLE] Установлен заголовок (текущие): {title}")
...
class MyApp(QMainWindow):
    ...
    def setup_history_controls(self):
        ...
        if hasattr(self, 'tabBook'):
            ...
            # Если tabBook еще не создан, создаем центральный виджет с layout
            central_widget = QWidget()
            main_layout = QVBoxLayout()
            main_layout.addLayout(self.history_control_layout)
    ...
    def load_historical_data(self):
        ...
        # Приостанавливаем таймер проверки файлов
        self.tabWidgets.pause_refresh_timer()
    ...
    def load_current_data(self):
        ...
        # Возобновляем таймер проверки файлов
        self.tabWidgets.resume_refresh_timer()
    ...
    def update_title_bar(self):
        """Обновляет заголовок окна в зависимости от режима."""
        # В любом случае используем метод из виджетов, который теперь учитывает исторический режим
